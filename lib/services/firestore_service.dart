...
class FirestoreService {
  ...
  Future<CategoryModel> createCategory(String name) async {
    // Получаем все категории для определения следующего ID
    final snapshot = await _categoriesCollection.orderBy('id', descending: true).limit(1).get();
    
    int nextId;
    if (snapshot.docs.isEmpty) {
      nextId = 4; // Начальный ID после стандартных категорий
    } else {
  ...
  // Инициализация стандартных категорий (вызывать один раз при первом запуске)
